    ...
    if ($_POST["action"] == "clearcache") {
        ...
        if (isset($_FILES['file']['name'])) {
            if (!$_FILES['file']['error']) {
                ...
                $filename = $name . '.' . $ext[1];
                $destination = system\Helper::arcGetPath(true) . "app/modules/blog/images/" . $filename; //change this directory
                $location = $_FILES["file"]["tmp_name"];
                move_uploaded_file($location, $destination);
                echo json_encode(["data" => system\Helper::arcGetPath() . "app/modules/blog/images/" . $filename, "status" => "success"]);
            } else {
