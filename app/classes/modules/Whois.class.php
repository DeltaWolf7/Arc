 ...
 * Usage:
 * include 'Whois.php';
 * $domain = new Whois("mydomain.com");
 * echo $domain->expiryDate();
 * 
 * Extended version of https://github.com/regru/php-whois
 * Written by Vaneska, extended by Craig Longford
 */
...
class Whois {
     ...
     * @return string registered date
     */
    public function registeredDate() {
        return self::processData(2);
    }
     ...
     * @return date
     */
    private function processData($type = 3) {
        $lookup = "";
        if (isset($this->servers[$this->TLDs][$type])) {
            $lookup = $this->servers[$this->TLDs][$type];
        } else {
            return "Type not defined for TLD '{$type}'";
        }

        $lookup_string = preg_match("/" . $lookup . ": (.*)/i", $this->info(), $matches);
        return $matches[1];
    }
    ...
    public function isValid() {
        if (isset($this->servers[$this->TLDs][0]) && strlen($this->servers[$this->TLDs][0]) > 6) {
            ...
            if (preg_match("/^[a-z0-9\-]{3,}$/", $tmp_domain) && !preg_match("/^-|-$/", $tmp_domain)) {
                return true;
            }
        }
        return false;
